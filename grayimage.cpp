...
tResult image::Init(tInitStage eStage, __exception)
{
	...
	if (eStage == StageFirst)
	{
		...
		RETURN_IF_FAILED(createVideoOutputPin("videoOutput", this->VideoOutputPin));
		RETURN_IF_FAILED(createVideoOutputPin("display", this->videoDisplay));
		...
		videoDisplay.SetFormat(&videoOutputInfo, NULL);
	}
...
tResult image::OnPinEvent(IPin *psource, tInt eventCore, tInt param1, tInt param2, IMediaSample *mediaSample)
{
	...
	if (eventCore == IPinEventSink::PE_MediaSampleReceived)
	{
		if (psource == &this->videoInput)
		{
			...
			TransmitOutput(mediaSample->GetTime(), this->videoDisplay);
			TransmitOutput(mediaSample->GetTime(), this->VideoOutputPin);
...
tResult image::TransmitOutput(const tTimeStamp &timeStamp, cVideoPin &pin)
{
	...
	//Transmit to output with data of gray image ==videoOutputInfo
